#!/usr/bin/env python3
"""
Headed product crawl: same as crawl_products.py but runs with a visible browser (--headed).
Use this to test if timeouts disappear when the site sees a real window instead of headless.

Usage:
  python crawl_products_headed.py --headed --limit 10 saigoncenter_en_sitemap.xml --out batch_headed.json

Options: same as crawl_products.py, plus:
  --headed    Show browser window (headless=False). Often avoids timeouts when site blocks headless.
"""
...
def crawl(
    ...
    *,
    ...
    if headed:
        print("Running with visible browser (--headed).", file=sys.stderr)
    elif not use_chrome:
        print("Tip: if many timeouts, try --use-chrome or run with --headed (this script).", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Crawl product pages (headed variant: use --headed to show browser window)."
    )
    parser.add_argument(
        "sitemaps",
        nargs="*",
    ...
    parser.add_argument("--headed", action="store_true", help="Show browser window (headless=False); often fixes timeouts")
